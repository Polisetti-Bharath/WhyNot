  ...
  const fetchSingleAnalysis = async () => {
    ...
    try {
        ...
        ? snapshotSkills
            .map((s: any) => ({
              ...
              evidence: Array.isArray(s?.evidence)
                ? s.evidence.map((e: any) => e?.type || 'Unknown')
                : [],
            }))
      ...
      const reqData = {
        ...
        jobMinCgpa:
          typeof (job.minCgpa || job.min_cgpa) === 'number' ? job.minCgpa || job.min_cgpa : 0,
          ...
          .map(app => {
            ...
            return {
              ...
              jobRequiredSkills: Array.isArray(requiredSkills)
                ...
                : [],
              jobMinCgpa:
                typeof (job?.minCgpa || job?.min_cgpa) === 'number'
...
Sentiment: ${explanation.sentiment}
    ...
    const exportText = formatAnalysisForExport(student.name, explanationText, {
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">
                  {mode === 'bulk'
                    ...
                    : application
                      ? `${application.job.role} @ ${application.job.company}`
                      : ''}
                </p>
                {mode === 'single' && application && (
                  <>
                    <div className="mt-1 flex items-center gap-1 text-xs text-purple-400">
                      ...
                      <span>
                        Decision-specific analysis (takes precedence over general resume feedback)
                      </span>
                    </div>
            ...
            {mode === 'bulk' && applications.filter(a => a.status === 'REJECTED').length === 0 && (
              <div className="flex flex-col items-center justify-center h-full py-16 px-8 text-center">
                ...
                <p className="text-slate-400 max-w-md mb-6">
                  You don&apos;t have any rejections yet. Keep applying to opportunities and
                  we&apos;ll help you analyze patterns if needed.
                </p>
                <button
                  onClick={onClose}
                ...
                <div className="p-8 space-y-6 border-r border-white/10 bg-slate-900/50">
                  ...
                  <div className="space-y-3">
                    ...
                    {(
                      ...
                      (application.job as any).required_skills ||
                      []
                    ).map((reqSkill: any) => {
                    ...
                    <div className="flex items-center justify-between p-4 rounded-lg bg-white/5 border border-white/5">
                      <div>
                        ...
                        <span className="text-lg font-bold text-white">
                          {application.job.minCgpa || (application.job as any).min_cgpa}
                        </span>
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        <span
                          className={`text-lg font-bold ${student.cgpa >= (application.job.minCgpa || (application.job as any).min_cgpa) ? 'text-green-400' : 'text-red-400'}`}
                        >
                          {student.cgpa}
                        </span>
                      </div>
                    ...
                    {loading ? (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          >
                            {explanation.type === 'RULE_BASED'
                              ? '⚠️ Rule-Based Rejection'
                              : 'ℹ️ Non-Rule-Based Rejection'}
                          </span>
                        </div>
                        ...
                        {explanation.type === 'NON_RULE_BASED' && (
                          <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                            ...
                            <p className="text-sm text-slate-300">
                              You met the listed eligibility criteria. The rejection may be due to
                              limited openings, internal preferences, or company-side screening
                              processes. This is subjective and doesn't necessarily reflect your
                              qualifications.
                            </p>
                          </div>
                        )}
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {explanation.keyMissingSkills.map((skill, idx) => (
                                <span
                                  key={idx}
                                  className="px-3 py-1 bg-red-500/10 border border-red-500/20 rounded-full text-sm text-red-300"
                                >
                                  {skill}
                                </span>
                              ))}
                        ...
                        <div className="p-4 bg-neon-purple/10 border border-neon-purple/20 rounded-lg">
                          ...
                          <p className="text-sm text-slate-300 italic">"{explanation.sentiment}"</p>
                        </div>

                        {/* Trust Disclaimer */}
                        <div className="p-3 bg-slate-800/50 border border-slate-700/50 rounded-lg">
                          <p className="text-xs text-slate-400 italic text-center">
                            This explanation is based on declared profile data and listed
                            eligibility criteria. Final hiring decisions may include additional
                            factors.
                          </p>
                        </div>
                      ...
                      {patternAnalysis.cgpaIssues && (
                        <div className="glass-panel p-6 rounded-xl border border-yellow-500/30 bg-yellow-500/5">
                          ...
                          <p className="text-slate-300">
                            Your CGPA is consistently below the minimum requirements for many
                            opportunities. Focus on academic improvement alongside skill
                            development.
                          </p>
                        </div>
                      )}
